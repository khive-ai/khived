...
@pytest.fixture
def client(storage_client_config, mock_boto_s3_constructor: MagicMock): # Corrected dependency
...
def test_object_storage_client_initialization(storage_client_config, mock_boto_s3_constructor: MagicMock):
    """Test client initialization and that boto3.client is called correctly."""
    ObjectStorageClient(**storage_client_config) # Instantiate to trigger the constructor
...
def test_upload_object_success(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    object_name = "test_obj.txt"
    data = b"test data"
    metadata = {"custom": "value"}
    content_type = "text/plain"

    result = client.upload_object(object_name, data, metadata=metadata, content_type=content_type)
...
def test_upload_object_client_error(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.upload_fileobj.side_effect = ClientError({"Error": {"Code": "SomeError", "Message": "Details"}}, "upload_fileobj")
    result = client.upload_object("test.txt", b"data")
    assert result is False

def test_upload_object_unexpected_error(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.upload_fileobj.side_effect = Exception("Unexpected")
    result = client.upload_object("test.txt", b"data")
    assert result is False

def test_download_object_success(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    object_name = "test_obj.txt"
    ...
    mock_s3_client_instance.get_object.assert_called_once_with(Bucket=client.bucket_name, Key=object_name)

def test_download_object_no_such_key(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.get_object.side_effect = ClientError({"Error": {"Code": "NoSuchKey", "Message": "Not found"}}, "get_object")
    result = client.download_object("non_existent.txt")
    assert result is None

def test_download_object_client_error(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.get_object.side_effect = ClientError({"Error": {"Code": "SomeError", "Message": "Details"}}, "get_object")
    result = client.download_object("test.txt")
    assert result is None

def test_download_object_unexpected_error(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.get_object.side_effect = Exception("Unexpected")
    result = client.download_object("test.txt")
    assert result is None

def test_get_presigned_url_get_success(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    object_name = "test_obj.txt"
...
def test_get_presigned_url_put_success(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
...
def test_get_presigned_url_unsupported_method(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
...
def test_get_presigned_url_client_error(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.generate_presigned_url.side_effect = ClientError({"Error": {"Code": "SomeError", "Message": "Details"}}, "generate_presigned_url")
    result = client.get_presigned_url("test.txt")
    assert result is None

def test_get_presigned_url_unexpected_error(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.generate_presigned_url.side_effect = Exception("Unexpected")
    result = client.get_presigned_url("test.txt")
    assert result is None

def test_object_exists_true(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.head_object.return_value = {}  # Success, no error
    result = client.object_exists("existing_obj.txt")
    assert result is True
    mock_s3_client_instance.head_object.assert_called_once_with(Bucket=client.bucket_name, Key="existing_obj.txt")

def test_object_exists_false_404(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.head_object.side_effect = ClientError({"Error": {"Code": "404", "Message": "Not Found"}}, "head_object")
    result = client.object_exists("non_existing_obj.txt")
    assert result is False

def test_object_exists_client_error(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.head_object.side_effect = ClientError({"Error": {"Code": "SomeError", "Message": "Details"}}, "head_object")
    result = client.object_exists("test.txt")
    assert result is False # Or True/raise depending on how you want to handle non-404 errors

def test_object_exists_unexpected_error(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.head_object.side_effect = Exception("Unexpected")
    result = client.object_exists("test.txt")
    assert result is False

def test_ensure_bucket_exists_already_exists(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.head_bucket.return_value = {} # Bucket exists
    result = client.ensure_bucket_exists()
    assert result is True
    mock_s3_client_instance.head_bucket.assert_called_once_with(Bucket=client.bucket_name)
    mock_s3_client_instance.create_bucket.assert_not_called()

def test_ensure_bucket_exists_creates_new_bucket_no_region_constraint(client: ObjectStorageClient, mock_s3_client_instance: MagicMock, storage_client_config, mock_boto_s3_constructor: MagicMock):
    # Simulate bucket not found, then successful creation
    mock_s3_client_instance.head_bucket.side_effect = ClientError({"Error": {"Code": "404", "Message": "Not Found"}}, "head_bucket")
    ...
    result = client_us_east_1.ensure_bucket_exists() # This will use the mock_s3_client_instance associated with client_us_east_1
    assert result is True
    ...
    s3_instance_for_client_us_east_1.head_bucket.assert_called_with(Bucket=client_us_east_1.bucket_name)
    ...
    for c_name, c_args, c_kwargs in s3_instance_for_client_us_east_1.method_calls:
        if c_name == 'create_bucket' and c_kwargs.get('Bucket') == client_us_east_1.bucket_name:
            create_bucket_call_args = c_kwargs
            break
    assert create_bucket_call_args is not None
    assert create_bucket_call_args.get('CreateBucketConfiguration') is None # No constraint for us-east-1


def test_ensure_bucket_exists_creates_new_bucket_with_region_constraint(client: ObjectStorageClient, mock_s3_client_instance: MagicMock, storage_client_config, mock_boto_s3_constructor: MagicMock):
    ...
    s3_client_for_other_region.head_bucket.side_effect = ClientError({"Error": {"Code": "404", "Message": "Not Found"}}, "head_bucket")
    ...
    for c_name, c_args, c_kwargs_dict in reversed(s3_client_for_other_region.method_calls):
        if c_name == 'create_bucket' and c_kwargs_dict.get('Bucket') == client_other_region.bucket_name:
            create_bucket_call_kwargs = c_kwargs_dict
            break
    assert create_bucket_call_kwargs is not None
    assert create_bucket_call_kwargs.get('CreateBucketConfiguration') == {'LocationConstraint': 'eu-west-1'}


def test_ensure_bucket_exists_creation_fails(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.head_bucket.side_effect = ClientError({"Error": {"Code": "404", "Message": "Not Found"}}, "head_bucket")
    mock_s3_client_instance.create_bucket.side_effect = ClientError({"Error": {"Code": "BucketAlreadyExists", "Message": "Cannot create"}}, "create_bucket")
    result = client.ensure_bucket_exists()
    assert result is False

def test_ensure_bucket_exists_head_bucket_fails_non_404(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.head_bucket.side_effect = ClientError({"Error": {"Code": "SomeOtherError", "Message": "Access Denied"}}, "head_bucket")
    result = client.ensure_bucket_exists()
    assert result is False
    mock_s3_client_instance.create_bucket.assert_not_called()

def test_ensure_bucket_exists_unexpected_error_on_head(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.head_bucket.side_effect = Exception("Network Error")
    result = client.ensure_bucket_exists()
    assert result is False

def test_ensure_bucket_exists_unexpected_error_on_create(client: ObjectStorageClient, mock_s3_client_instance: MagicMock):
    mock_s3_client_instance.head_bucket.side_effect = ClientError({"Error": {"Code": "404", "Message": "Not Found"}}, "head_bucket")
    mock_s3_client_instance.create_bucket.side_effect = Exception("Unexpected error during creation")
    result = client.ensure_bucket_exists()
    assert result is False