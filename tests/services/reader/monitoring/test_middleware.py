...
def mock_http_requests_total():
    ...
    with patch(
        "khive.reader.monitoring.middleware.HTTP_REQUESTS_TOTAL"
    ) as mock_counter:
...
def mock_http_requests_latency_seconds():
    # Path to the metric defined *within* middleware.py
    with patch(
        "khive.reader.monitoring.middleware.HTTP_REQUESTS_LATENCY_SECONDS"
    ) as mock_histogram:
...
async def client():
    ...
    if not any(
        isinstance(mw.middleware, PrometheusMiddleware)
