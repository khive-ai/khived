...
@pytest.mark.parametrize(
    "metric_name, metric_counter, metric_histogram, has_status_label",
    [
        ...
        (
            ...
            EMBEDDING_LATENCY,
            False,
        ),  # Embedding only has latency histogram in current spec
    ],
)
async def test_monitor_async_decorator(
    metric_name, metric_counter, metric_histogram, has_status_label
):
    """
    ...
    if metric_counter:
        if has_status_label:
            patch_target_counter = (
                f"khive.reader.monitoring.prometheus.{metric_counter._name}.labels"
            )
        else:  # Should not happen with current spec for counters
            patch_target_counter = (
                f"khive.reader.monitoring.prometheus.{metric_counter._name}.inc"
            )
    if metric_histogram:
        patch_target_histogram = (
            f"khive.reader.monitoring.prometheus.{metric_histogram._name}.observe"
        )
    ...
    with patch(
        patch_target_histogram
        if metric_histogram
        else "khive.reader.monitoring.prometheus.asyncio.sleep",
        mock_histogram_observe if metric_histogram else AsyncMock(),
    ):  # Patch histogram or a dummy if no histogram
...
async def test_update_metrics_periodically_updates_gauges():
    ...
    with (
        patch.object(VECTOR_COUNT, "set") as mock_vector_set,
        ...
        with patch("builtins.print") as mock_print:
            with pytest.raises(asyncio.CancelledError):  # Expect loop to be cancelled
                await update_metrics_periodically(
                    mock_db_session_factory, mock_task_queue
                )
...
async def test_update_metrics_periodically_handles_errors():
    ...
    with (
    ...
    ):  # Patch print to check for error logging
        mock_sleep.side_effect = (
            asyncio.CancelledError
        )  # Stop loop after first iteration
        ...
        assert any(
            "Error updating metrics: DB connection error" in call.args[0]
            for call in mock_print.call_args_list
        )
        mock_sleep.assert_called_once_with(
            60
        )  # Ensure sleep is still called before loop breaks
...
def test_start_metrics_server():
    ...
    with (
        patch("prometheus_client.start_http_server") as mock_start_http_server,
        patch("builtins.print") as mock_print,
    ):
