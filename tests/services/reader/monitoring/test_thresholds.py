...
import pytest
from khive.reader.monitoring.thresholds import PerformanceMonitor, PerformanceThreshold
...
# Test UT-T1
@pytest.mark.asyncio
async def test_performance_threshold_check_below_threshold(
    basic_threshold, mock_check_function
):
...
# Test UT-T2
@pytest.mark.asyncio
async def test_performance_threshold_check_exceeds_threshold_first_time(
    basic_threshold, mock_check_function, mock_alert_handler
):
    ...
    mock_alert_handler.assert_called_once_with(
        "test_threshold", "A test threshold", 150.0, 100.0
    )


@pytest.mark.asyncio
async def test_performance_threshold_check_remains_exceeded(
    basic_threshold, mock_check_function, mock_alert_handler
):
...
# Test UT-T3
@pytest.mark.asyncio
async def test_performance_threshold_check_recovers_from_exceeded(
    basic_threshold, mock_check_function, mock_alert_handler, caplog
):
...
# Test UT-T4
@pytest.mark.asyncio
async def test_performance_threshold_check_handles_exception_in_check_function(
    basic_threshold, mock_check_function, caplog
):
    ...
    assert (
        "Error checking threshold test_threshold: Check function error" in caplog.text
    )
...
def performance_monitor(mock_db_session_factory, mock_task_queue):
    ...
    with patch.object(
        PerformanceMonitor, "default_alert_handler", new_callable=AsyncMock
    ) as mock_default_alert:
        monitor = PerformanceMonitor(mock_db_session_factory, mock_task_queue)
        monitor.default_alert_handler_mock = (
            mock_default_alert  # Attach mock for assertions
        )
        return monitor
...
async def test_performance_monitor_check_all_thresholds(performance_monitor, caplog):
    ...
    performance_monitor._check_search_latency = AsyncMock(
        return_value=50.0
    )  # Below 100ms
    performance_monitor._check_task_queue_depth = AsyncMock(
        return_value=1500.0
    )  # Above 1000
    performance_monitor._check_db_connection_utilization = AsyncMock(
        return_value=0.95
    )  # Above 0.9
    ...
    performance_monitor.default_alert_handler_mock.assert_any_call(
        "task_queue_depth", "Number of pending tasks in the queue", 1500.0, 1000.0
    )
...
async def test_performance_monitor_placeholder_check_functions(performance_monitor):
    ...
    assert (
        await monitor._check_db_connection_utilization() == 0.5
    )  # Default placeholder
...
async def test_performance_monitor_monitor_periodically(performance_monitor, caplog):
    ...
    with (
        patch.object(
            performance_monitor, "check_all_thresholds", new_callable=AsyncMock
        ) as mock_check_all,
        ...
        assert (
            "Performance monitor starting periodic checks every 10 seconds."
            in caplog.text
        )


@pytest.mark.asyncio
async def test_performance_monitor_monitor_periodically_handles_check_error(
    performance_monitor, caplog
):
    ...
    with (
        ...
        mock_check_all.assert_called_once()
        assert (
            "Error during periodic threshold check: Periodic check failed"
            in caplog.text
        )
        mock_sleep.assert_called_once_with(10)
