...
from khive.services.reader.utils import dir_to_files, calculate_text_tokens
...
def test_dir_to_files_with_specific_types_recursive(sample_dir_structure):
    ...
    files = dir_to_files(sample_dir_structure, file_types=[".txt", ".md"], recursive=True)
    paths = {f.name for f in files}
...
@pytest.mark.skip(reason="Test is too complex for mock environment; relies on implementation details that might change")
