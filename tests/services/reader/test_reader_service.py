...
from khive.services.reader.reader_service import ReaderService, DOCLING_SUPPORTED_FORMATS
...
def mock_docling_converter(mocker):
    ...
    mocker.patch("docling.document_converter.DocumentConverter", return_value=mock_converter_instance)
...
def mock_tempfile(mocker):
    ...
    mock_named_temp_file = mocker.patch("khive.services.reader.reader_service.tempfile.NamedTemporaryFile")
...
def reader_service_instance(mock_docling_converter):
    ...
    with patch("khive.services.reader.reader_service.calculate_text_tokens", return_value=5):
...
def test_handle_request_with_open_action(reader_service_instance):
    ...
    with patch.object(reader_service_instance, "_open_doc") as mock_open_doc:
        ...
        request = ReaderRequest(
            ...
            params=ReaderOpenParams(path_or_url="test.pdf")
        )
...
def test_handle_request_with_read_action(reader_service_instance):
    ...
    with patch.object(reader_service_instance, "_read_doc") as mock_read_doc:
        ...
        request = ReaderRequest(
            ...
            params=ReaderReadParams(doc_id="DOC_123")
        )
...
def test_open_doc_local_file_success(reader_service_instance, mock_tempfile, mock_docling_converter):
...
@pytest.mark.skip(reason="Dependency issues in test environment")
def test_open_doc_url_success(reader_service_instance, mock_tempfile, mock_docling_converter):
