...
from khive.protocols.types import Execution, ExecutionStatus, Log
...
async def test_as_event_custom_request_arg(mock_adapter):
    ...
    with patch('khive.protocols.event.validate_model_to_dict') as mock_validate:
        ...
        mock_validate.side_effect = lambda x: (
            {"value": "from_custom"} if isinstance(x, dict) and "value" in x else {"input": "ignored"}
        )
...
async def test_as_event_with_class_method():
    ...
    with patch('khive.protocols.event.validate_model_to_dict', side_effect=lambda x: x if isinstance(x, dict) else {"input": "test"}):
