...
async def test_check_performance_success():
    ...
    with (
        patch(
            f"{CLI_MODULE_PATH}.PerformanceMonitor", return_value=mock_monitor_instance
        ) as mock_perf_monitor_class,
        patch(
            f"{CLI_MODULE_PATH}.get_db_session", new_callable=AsyncMock
        ) as mock_get_db,
        patch(f"{CLI_MODULE_PATH}.task_queue", MagicMock()),
    ):  # Mock task_queue if it's directly used by check_performance
        from khive.cli.khive_reader import (
            check_performance,  # Import here to use patched versions
        )
...
@patch(
    f"{CLI_MODULE_PATH}.asyncio.run"
)  # Mock asyncio.run to control what check_performance returns
...
def test_cli_performance_command_success_no_exceed(
    mock_sys_exit, mock_print, mock_asyncio_run
):
    ...
    assert (
        "vector_count\033[0m: 1000.00 / 5000000.00 [OK]" in output
    )  # Check for ANSI reset too
...
def test_cli_performance_command_success_exceeded(
    mock_sys_exit, mock_print, mock_asyncio_run
):
...
@patch("builtins.print")
@patch(f"{CLI_MODULE_PATH}.sys.exit")
def test_cli_performance_command_json_output(
    mock_sys_exit, mock_print, mock_asyncio_run
):
...
@patch(f"{CLI_MODULE_PATH}.sys.exit")
def test_cli_performance_command_error_status(
    mock_sys_exit, mock_stderr_write, mock_asyncio_run
):
