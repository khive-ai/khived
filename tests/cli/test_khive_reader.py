...
from khive.reader.services.ingestion_service import Document as IngestDocumentModel, DocumentStatus
...
def mock_ingestion_service_components():
    ...
    with patch('khive.cli.khive_reader.ObjectStorageClient', return_value=mock_osc) as patched_osc, \
         patch('khive.cli.khive_reader.InMemoryDocumentRepository', return_value=mock_dr) as patched_dr, \
         patch('khive.cli.khive_reader.DocumentIngestionService') as patched_dis_constructor:
...
def test_ingest_command_success_no_metadata(mock_ingestion_service_components):
    ...
    mock_service_instance = mock_ingestion_service_components["DocumentIngestionServiceInstance"]
...
def test_ingest_command_success_with_metadata(mock_ingestion_service_components, tmp_path: Path):
    ...
    with patch.dict(importlib.import_module('os').environ, MOCK_MINIO_ENV):
        result = runner.invoke(reader_cli_app, ["ingest", "--source-uri", source_uri, "--metadata-file", str(metadata_file)])
...
def test_ingest_command_success_json_output(mock_ingestion_service_components):
    ...
    with patch.dict(importlib.import_module('os').environ, MOCK_MINIO_ENV):
        result = runner.invoke(reader_cli_app, ["ingest", "--source-uri", source_uri, "--json-output"])
...
def test_ingest_command_invalid_uri(mock_ingestion_service_components):
    ...
    assert "Invalid source URI" in result.stdout # Typer prints to stdout for errors by default with secho
    mock_ingestion_service_components["DocumentIngestionServiceInstance"].ingest_document_from_url.assert_not_called()

def test_ingest_command_metadata_file_not_found(mock_ingestion_service_components):
    source_uri = "http://example.com/doc.nf"
    with patch.dict(importlib.import_module('os').environ, MOCK_MINIO_ENV):
        result = runner.invoke(reader_cli_app, ["ingest", "--source-uri", source_uri, "--metadata-file", "non_existent.json"])

    assert result.exit_code == 1
    assert "Metadata file not found" in result.stdout

def test_ingest_command_malformed_metadata_json(mock_ingestion_service_components, tmp_path: Path):
...
def test_ingest_command_ingestion_service_fails(mock_ingestion_service_components):
    ...
    mock_service_instance = mock_ingestion_service_components["DocumentIngestionServiceInstance"]
    mock_service_instance.ingest_document_from_url.return_value = None # Simulate failure
...
def test_ingest_command_ingestion_service_raises_exception(mock_ingestion_service_components):
    source_uri = "http://example.com/doc.exception"
    mock_service_instance = mock_ingestion_service_components["DocumentIngestionServiceInstance"]
    mock_service_instance.ingest_document_from_url.side_effect = Exception("Internal service error")

    with patch.dict(importlib.import_module('os').environ, MOCK_MINIO_ENV):
        result = runner.invoke(reader_cli_app, ["ingest", "--source-uri", source_uri])
    
    assert result.exit_code == 1
    assert "An error occurred during ingestion: Exception: Internal service error" in result.stdout
...
def test_ingest_command_minio_config_missing(mock_ingestion_service_components):
    ...
    with patch.dict(importlib.import_module('os').environ, incomplete_env, clear=True):
        ...
        result = runner.invoke(reader_cli_app, ["ingest", "--source-uri", "http://example.com/doc.miniofail"])
...
async def test_open_command_callable(MockReaderServiceGroup):
    ...
    mock_service_instance.handle_request = AsyncMock(return_value=ReaderResponse(
        success=True,
...
async def test_read_command_callable(MockReaderServiceGroup):
    ...
    assert result.exit_code == 0
    assert '"text_slice": "sample text"' in result.stdout # Assuming model_dump uses text_slice for PartialChunk.content
...
async def test_list_command_callable(MockReaderServiceGroup):
    ...
    mock_service_instance.handle_request = AsyncMock(return_value=ReaderResponse(
        ...
        content=ReaderListDirResponseContent( # This was ReaderOpenResponseContent before, fixed
            ...
            doc_info=DocumentInfo(doc_id="dir_doc", length=50, num_tokens=5) # list_dir also creates a doc
        )
    ))
