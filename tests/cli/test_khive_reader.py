...
def run_reader_cli_with_args(monkeypatch, args_list, mock_service_call=None):
    ...
    monkeypatch.setattr(
        "khive.services.reader.reader_service.ReaderService", lambda: mock_service
    )
    ...
    monkeypatch.setattr(
        "khive.cli.khive_reader._load_cache", MagicMock(return_value={})
    )
    ...
    try:
        ...
        print(
            f"Unexpected exception in run_reader_cli_with_args: {type(e).__name__}: {e}"
        )
...
def test_cli_open_success(monkeypatch, mock_service_response):
    ...
    mock_service_response.return_value = ReaderResponse(
        success=True, content=ReaderOpenResponseContent(doc_info=doc_info)
    )

    # Set up the service documents dict for caching
    mock_service_docs = {"DOC_123": ("/tmp/fake_doc.txt", 1000)}
    monkeypatch.setattr(
        "khive.cli.khive_reader.reader_service.documents", mock_service_docs
    )

    cli_args = ["open", "--path_or_url", "test.pdf"]
    mock_print, mock_exit = run_reader_cli_with_args(
        monkeypatch, cli_args, mock_service_response
    )
...
def test_cli_open_failure(monkeypatch, mock_service_response):
    ...
    mock_service_response.return_value = ReaderResponse(
        success=False, error="Could not open file: file not found", content=None
    )
...
def test_cli_read_success(monkeypatch, mock_service_response):
    ...
    mock_service_response.return_value = ReaderResponse(
        success=True, content=ReaderReadResponseContent(chunk=chunk)
    )
...
def test_cli_read_with_cache(monkeypatch):
    ...
    cache_data = {
        "DOC_456": {"path": "/tmp/cached_doc.txt", "length": 500, "num_tokens": 100}
    }
    monkeypatch.setattr(
        "khive.cli.khive_reader._load_cache", MagicMock(return_value=cache_data)
    )
    ...
    monkeypatch.setattr(
        "khive.cli.khive_reader.reader_service.handle_request", mock_response
    )
...
def test_cli_read_not_found(monkeypatch, mock_service_response):
    ...
    # Set up the mock service to return a failure response
    mock_service_response.return_value = ReaderResponse(
        success=False, error="doc_id not found in memory", content=None
    )
