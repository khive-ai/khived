...
class Document(DocumentBase):
    ...
    storage_path: Optional[str] = None # e.g., s3_bucket/document_id/raw_content.ext
...
class DocumentRepository(Protocol):
    ...
    async def update_document_status(
        self, document_id: uuid.UUID, status: DocumentStatus, error_message: Optional[str] = None
    ) -> Optional[Document]:
        ...

    @abstractmethod
    async def update_document_storage_path(
        self, document_id: uuid.UUID, storage_path: str, size_bytes: Optional[int] = None
    ) -> Optional[Document]:
        ...
...
class DocumentIngestionService:
    ...
    async def _download_content_from_url(self, url: HttpUrl) -> Optional[bytes]:
        ...
        try:
            ...
            logger.info(f"Successfully downloaded content from URL: {url}, status: {response.status_code}")
            return response.content
        except requests.exceptions.RequestException as e:
            logger.error(f"Failed to download content from URL '{url}': {e}", exc_info=True)
            return None
        except Exception as e:
            logger.error(f"An unexpected error occurred while downloading from URL '{url}': {e}", exc_info=True)
            return None
    ...
    async def _queue_document_for_processing(self, document: Document) -> bool:
        ...
        if self.task_queue_client:
            ...
            logger.info(f"Document '{document.id}' would be queued for processing (Task Queue TBD).")
            return True # Assume success for now
        else:
            logger.warning(f"Task queue client not available. Document '{document.id}' not queued for processing.")
    ...
    async def ingest_document_from_url(
        self,
        source_uri: HttpUrl,
        metadata_file_content: Optional[Dict[str, Any]] = None,
    ) -> Optional[Document]:
        """
        ...
        """
        doc_create = DocumentCreate(source_uri=source_uri, metadata=metadata_file_content or {})
        
        try:
            document = await self.document_repository.create_document(doc_create)
            logger.info(f"Created document record with ID: {document.id} for URI: {source_uri}")
        except Exception as e:
            logger.error(f"Failed to create document record for URI '{source_uri}': {e}", exc_info=True)
            return None

        # Update status to DOWNLOADING
        document = await self.document_repository.update_document_status(document.id, DocumentStatus.DOWNLOADING)
        if not document:
            logger.error(f"Failed to update document {document.id} status to DOWNLOADING.")
        ...
        if raw_content is None:
            logger.error(f"Failed to download content for document ID: {document.id} from {source_uri}")
            error_document = await self.document_repository.update_document_status(
                document.id, DocumentStatus.DOWNLOAD_FAILED, error_message="Failed to download content from source URI."
            )
        ...
        file_extension = source_uri.path.split('.')[-1] if '.' in source_uri.path else 'bin'
        ...
        upload_successful = self.storage_client.upload_object(
            ...
            content_type=document.content_type or "application/octet-stream" # Fallback
        )

        if not upload_successful:
            logger.error(f"Failed to upload raw content to S3 for document ID: {document.id}")
            # Get the updated document after setting status to ERROR
            error_document = await self.document_repository.update_document_status(
                document.id, DocumentStatus.ERROR, error_message="Failed to store raw content in object storage."
            )
            return error_document # Return the document with the ERROR status

        logger.info(f"Successfully stored raw content for document ID: {document.id} at S3 path: {storage_object_name}")
        original_document_id = document.id # Save ID before potential None
        updated_doc_after_storage_path = await self.document_repository.update_document_storage_path(
            document.id, storage_path=f"{self.storage_client.bucket_name}/{storage_object_name}", size_bytes=document.size_bytes
        )
        
        if not updated_doc_after_storage_path:
            logger.error(f"Failed to update document {original_document_id} with storage path in repository.")
            ...
            error_doc = await self.document_repository.update_document_status(
                original_document_id, DocumentStatus.ERROR, error_message="Failed to update document with storage path."
            )
        ...
        updated_doc_after_status = await self.document_repository.update_document_status(document.id, DocumentStatus.CONTENT_STORED)
        if not updated_doc_after_status:
            logger.error(f"Failed to update document {document.id} status to CONTENT_STORED.")
        ...
        if queued_successfully:
            ...
            document = await self.document_repository.update_document_status(document.id, DocumentStatus.QUEUED_FOR_PROCESSING)
            if not document:
                 logger.error(f"Failed to update document {document.id} status to QUEUED_FOR_PROCESSING.")
            ...
            document = await self.document_repository.update_document_status(
                document.id, DocumentStatus.ERROR, error_message="Failed to queue document for processing."
            )
            if not document:
                logger.error(f"Failed to update document {document.id} status to ERROR after queue failure.")
                return document # Return with previous status
...
class InMemoryDocumentRepository(DocumentRepository):
    ...
    async def get_document_by_id(self, document_id: uuid.UUID) -> Optional[Document]:
        ...
        logger.info(f"(InMemoryRepo) Get document by ID {document_id}: {'Found' if doc else 'Not Found'}")
    ...
    async def update_document_status(
        ...
        if document_id in self._documents:
            ...
            doc.updated_at = datetime.now(timezone.utc)
            logger.info(f"(InMemoryRepo) Updated document {document_id} status to {status}")
            return doc.copy(deep=True)
        logger.warning(f"(InMemoryRepo) Update status: Document {document_id} not found.")
        return None
    ...
    async def update_document_storage_path(
        ...
        if document_id in self._documents:
            ...
            doc.updated_at = datetime.now(timezone.utc)
            logger.info(f"(InMemoryRepo) Updated document {document_id} storage path to {storage_path}")
            return doc.copy(deep=True)
        logger.warning(f"(InMemoryRepo) Update storage path: Document {document_id} not found.")
        return None
...
if __name__ == "__main__":
    ...
    async def main_example():
        if not all([MINIO_ENDPOINT_URL, MINIO_ACCESS_KEY, MINIO_SECRET_KEY, MINIO_BUCKET_NAME]):
            logger.error("MinIO environment variables not fully set for example. Skipping.")
            return
        ...
        if not storage_client.ensure_bucket_exists():
            logger.error(f"Failed to ensure MinIO bucket '{MINIO_BUCKET_NAME}' exists. Aborting example.")
            return
        ...
        test_url = HttpUrl("https://raw.githubusercontent.com/khive-ai/khive.d/main/README.md")
        ...
        if ingested_doc:
            ...
            if ingested_doc.storage_path:
                ...
                obj_name_in_s3 = ingested_doc.storage_path.split(f"{MINIO_BUCKET_NAME}/")[-1]
                logger.info(f"Attempting to download {obj_name_in_s3} from S3 to verify...")
                s3_content = storage_client.download_object(obj_name_in_s3)
                if s3_content:
                    logger.info(f"Successfully downloaded {len(s3_content)} bytes from S3 for {obj_name_in_s3}.")
                    # logger.info(f"S3 Content (first 100 chars): {s3_content[:100].decode(errors='ignore')}")
                else:
                    logger.error(f"Failed to download {obj_name_in_s3} from S3 for verification.")
        else:
