        ...
        try:
            ...
            if value > self.threshold_value:
                if not self.exceeded: # Alert only on first exceed
                    self.exceeded = True
                    logger.warning(f"Performance threshold EXCEEDED: {self.name} (Value: {value} > Threshold: {self.threshold_value})")
                ...
                self.exceeded = False
                logger.info(f"Performance threshold RECOVERED: {self.name} (Value: {value} <= Threshold: {self.threshold_value})")
...
class PerformanceMonitor:
    ...
    async def _check_vector_count(self, **kwargs) -> float: # Added **kwargs to match signature
    ...
    async def _check_db_connection_utilization(self, **kwargs) -> float: # Added **kwargs
    ...
    async def default_alert_handler(self, name: str, description: str, value: float, threshold: float):
        """Handle threshold alerts by logging."""
        try:
            exceeded_by_percentage = ((value / threshold) - 1) * 100 if threshold > 0 else float('inf')
    ...
    async def check_all_thresholds(self) -> Dict[str, Any]:
        ...
        for threshold in self.thresholds:
            ...
            logger.debug(f"Threshold '{threshold.name}': Value={threshold.last_value}, Exceeded={threshold.exceeded}")
    ...
    async def monitor_periodically(self, interval_seconds: int = 300):
        ...
        logger.info(f"Performance monitor starting periodic checks every {interval_seconds} seconds.")
        while True:
            try:
                await self.check_all_thresholds()
            except Exception as e:
                logger.error(f"Error during periodic threshold check: {e}", exc_info=True)
