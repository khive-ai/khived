...
class TextExtractor:
    ...
    def extract_text(self, file_path: Path, mime_type: str) -> str:
        ...
        if not extractor:
            logger.warning(f"Unsupported MIME type for text extraction: {mime_type} for file {file_path}")
        ...
        try:
            ...
            logger.error(f"Failed to extract text from {file_path} (MIME: {mime_type}): {e}", exc_info=True)
    ...
    def _extract_pdf(self, file_path: Path) -> str:
        ...
        try:
                ...
                if reader.is_encrypted:
                    try:
                        ...
                        if reader.decrypt("") == PyPDF2.PasswordType.OWNER_PASSWORD:
                            logger.info(f"Decrypted PDF {file_path} with empty owner password.")
                        elif reader.decrypt("") == PyPDF2.PasswordType.USER_PASSWORD:
                             logger.info(f"Decrypted PDF {file_path} with empty user password.")
                        else:
                            logger.warning(f"PDF {file_path} is encrypted and could not be decrypted with empty password.")
                            # Depending on policy, could return empty string or raise specific error
                            return ""
                    except Exception as decrypt_exc:
                        logger.error(f"Failed to decrypt PDF {file_path}: {decrypt_exc}")
                        raise ValueError(f"Failed to decrypt PDF {file_path}") from decrypt_exc
            ...
            logger.error("PyPDF2 library is not installed. Please install it to extract text from PDF files.")
            raise ImportError("PyPDF2 library is not installed. Required for PDF extraction.")
        except PyPDF2.errors.PdfReadError as e:
            logger.error(f"Error reading PDF file {file_path}: {e}")
            raise ValueError(f"Invalid or corrupted PDF file: {file_path}") from e
        except Exception as e:
            logger.error(f"An unexpected error occurred during PDF extraction for {file_path}: {e}", exc_info=True)
    ...
    def _extract_docx(self, file_path: Path) -> str:
        ...
        try:
            ...
            logger.error("python-docx library is not installed. Please install it to extract text from DOCX files.")
            raise ImportError("python-docx library is not installed. Required for DOCX extraction.")
    ...
    def _extract_html(self, file_path: Path) -> str:
        ...
        try:
            ...
            with open(file_path, "r", encoding="utf-8", errors="ignore") as f: # Specify encoding
                html_content = f.read()
            return h.handle(html_content)
        except ImportError:
            logger.error("html2text library is not installed. Please install it to extract text from HTML files.")
            raise ImportError("html2text library is not installed. Required for HTML extraction.")
        except Exception as e:
            logger.error(f"Failed to extract text from HTML {file_path}: {e}", exc_info=True)
